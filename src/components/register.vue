      ...
      <div>
        <div>
          <van-cell-group>
              ...
              <van-field @blur="verificationPwd" v-model.trim="registerList.pwd" clearable type="password" placeholder="请输入密码" />
          ...
          <div class="buttomArea">
              <van-button @click="toRegister">注册</van-button>
          </div>
          <!--  手机号键盘 -->
          <van-number-keyboard
...
export default {
  ...
  data () {
    return {
      ...
      toRegisterUrl:this.$store.state.url+"user/addUser",
    }
  },
   mounted(){
     this.identifyCode=""
...
// 注册
    toRegister(){
      if(this.verification()==1){
         var reg={
          phone: parseInt(this.registerList.phone),
          pwd: this.registerList.pwd,
          type: 0
        }
        this.$axios.post(this.toRegisterUrl,reg).then(res => {
